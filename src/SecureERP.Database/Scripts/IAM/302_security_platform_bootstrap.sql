...
(N'AUTH.SESSION.REVOKE', N'AUTH', N'SecurityAdministrationController', N'RevokeSession', N'POST', N'/api/v1/security/sessions/{sessionId}/revoke', N'Revocacion explicita de sesion en el contexto actual.', N'AUTH.SESSION.REVOKE', 1, 1, 1, 0, 1, 1, 0, N'AUTH.SESSION', 1),
(N'SECURITY.USER.READ.LIST', N'SECURITY', N'SecurityAdministrationController', N'ListUsers', N'GET', N'/api/v1/security/users', N'Consulta de usuarios de seguridad en el contexto autorizado.', N'SECURITY.USER.READ', 1, 1, 1, 0, 0, 1, 0, N'SECURITY.USER', 1),
(N'SECURITY.USER.READ.ONE', N'SECURITY', N'SecurityAdministrationController', N'GetUserById', N'GET', N'/api/v1/security/users/{id}', N'Consulta puntual de usuario de seguridad en el contexto autorizado.', N'SECURITY.USER.READ', 1, 1, 1, 0, 0, 1, 0, N'SECURITY.USER', 1),
(N'AUDIT.SECURITY_EVENT.READ', N'AUDIT', N'SecurityEventsController', N'List', N'GET', N'/api/v1/audit/security-events', N'Consulta de eventos de seguridad y observabilidad.', N'AUDIT.SECURITY_EVENT.READ', 1, 1, 1, 0, 1, 1, 0, N'AUDIT.SECURITY_EVENT', 1),
