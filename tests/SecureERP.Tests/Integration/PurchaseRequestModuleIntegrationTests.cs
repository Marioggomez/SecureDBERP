...
public sealed class PurchaseRequestModuleIntegrationTests
{
    [Fact]
    public async Task PurchaseRequest_CreateDraft_ShouldSucceed()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.CREATE");
        ...
        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        CreatePurchaseRequestHandler create = new(repository, scope.ContextAccessor);
        GetPurchaseRequestByIdHandler get = new(repository);

        CreatePurchaseRequestResponse created = await create.HandleAsync(
            new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "Draft create test"));
        PurchaseRequestDto? fetched = await get.HandleAsync(created.PurchaseRequestId!.Value);
    ...
    [Fact]
    public async Task PurchaseRequest_List_ShouldOnlyReturnCurrentCompanyRows()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.READ");
        (_, ValidateSessionResult session) = await scope.LoginAndSelectCompanyAsync(withMfa: true);
        scope.ContextAccessor.SetCurrent(new RequestContext(session.TenantId, session.CompanyId, session.UserId, session.SessionId, Guid.NewGuid().ToString()));

        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        CreatePurchaseRequestHandler create = new(repository, scope.ContextAccessor);
        ListPurchaseRequestsHandler list = new(repository);

        await create.HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "Own company row"));
        long otherCompanyId = await CreateCompanyInTenantAsync(scope.ConnectionString, scope.TenantId, "PR_OTH");
        await InsertPurchaseRequestInCompanyAsync(scope.ConnectionString, scope.TenantId, otherCompanyId, scope.UserId);

        IReadOnlyList<PurchaseRequestListItemDto> rows = await list.HandleAsync();
        Assert.NotEmpty(rows);

        foreach (PurchaseRequestListItemDto row in rows)
        {
            PurchaseRequestDto? full = await new GetPurchaseRequestByIdHandler(repository).HandleAsync(row.PurchaseRequestId);
            Assert.NotNull(full);
    ...
    [Fact]
    public async Task PurchaseRequest_GetById_ShouldNotExposeOtherCompanyRow()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.READ");
        (_, ValidateSessionResult session) = await scope.LoginAndSelectCompanyAsync(withMfa: true);
        ...
        long otherRequestId = await InsertPurchaseRequestInCompanyAsync(scope.ConnectionString, scope.TenantId, otherCompanyId, scope.UserId);

        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        PurchaseRequestDto? result = await new GetPurchaseRequestByIdHandler(repository).HandleAsync(otherRequestId);
        Assert.Null(result);
    }

    [Fact]
    public async Task PurchaseRequest_Update_ShouldOnlyAllowDraft()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.UPDATE");
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.SUBMIT");
        (_, ValidateSessionResult session) = await scope.LoginAndSelectCompanyAsync(withMfa: true);
        scope.ContextAccessor.SetCurrent(new RequestContext(session.TenantId, session.CompanyId, session.UserId, session.SessionId, Guid.NewGuid().ToString()));

        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        CreatePurchaseRequestHandler create = new(repository, scope.ContextAccessor);
        UpsertPurchaseRequestDetailHandler upsert = new(repository, scope.ContextAccessor);
        SubmitPurchaseRequestHandler submit = new(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository);
        UpdatePurchaseRequestDraftHandler update = new(repository, scope.ContextAccessor);

        long requestId = (await create.HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "Update test"))).PurchaseRequestId!.Value;
        await upsert.HandleAsync(new UpsertPurchaseRequestDetailRequest(requestId, null, null, "Line", 1, 100, null));
        await submit.HandleAsync(new SubmitPurchaseRequestRequest(requestId));

        UpdatePurchaseRequestDraftResponse response = await update.HandleAsync(
            new UpdatePurchaseRequestDraftRequest(requestId, null, DateTime.UtcNow.Date, "Should fail"));

        Assert.False(response.IsSuccess);
        Assert.Equal("PURCHASE_REQUEST_UPDATE_NOT_ALLOWED", response.ErrorCode);
    }

    [Fact]
    public async Task PurchaseRequest_Submit_ShouldTransitionToSubmitted()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.CREATE");
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.UPDATE");
        ...
        long requestId = (await create.HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "Submit test"))).PurchaseRequestId!.Value;
        await upsert.HandleAsync(new UpsertPurchaseRequestDetailRequest(requestId, null, null, "Line", 2, 25, "CC-01"));
        SubmitPurchaseRequestResponse result = await submit.HandleAsync(new SubmitPurchaseRequestRequest(requestId));
        PurchaseRequestDto? row = await get.HandleAsync(requestId);
    ...
    [Fact]
    public async Task PurchaseRequest_Approve_ShouldRequirePermission()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        (_, ValidateSessionResult session) = await scope.LoginAndSelectCompanyAsync(withMfa: true);
        scope.ContextAccessor.SetCurrent(new RequestContext(session.TenantId, session.CompanyId, session.UserId, session.SessionId, Guid.NewGuid().ToString()));
        ...
        AuthorizationCheckResult decision = await evaluator.EvaluateAsync(
            new AuthorizationCheckRequest(
                ...
                true,
                "/api/v1/purchase/requests/1/approve",
                "POST",
                "127.0.0.1",
                "SecureERP.Tests",
    ...
    [Fact]
    public async Task PurchaseRequest_Approve_ShouldRequireMfa()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.APPROVE");
    ...
    [Fact]
    public async Task PurchaseRequest_Approve_ShouldDenyCreatorBySoD()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.CREATE");
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.UPDATE");
        ...
        UpsertPurchaseRequestDetailHandler upsert = new(repository, scope.ContextAccessor);
        SubmitPurchaseRequestHandler submit = new(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository);
        ApprovePurchaseRequestHandler approve = new(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository);

        long requestId = (await create.HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "SoD"))).PurchaseRequestId!.Value;
        await upsert.HandleAsync(new UpsertPurchaseRequestDetailRequest(requestId, null, null, "Line", 1, 10, null));
        await submit.HandleAsync(new SubmitPurchaseRequestRequest(requestId));

        ApprovePurchaseRequestResponse result = await approve.HandleAsync(new ApprovePurchaseRequestRequest(requestId, "creator attempt"));
        Assert.False(result.IsSuccess);
        Assert.Equal("PURCHASE_REQUEST_SOD_DENY", result.ErrorCode);
    }

    [Fact]
    public async Task PurchaseRequest_Approve_ShouldPersistAudit_WhenApproved()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        ...
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.UPDATE");
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.SUBMIT");
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, approverUserId, "PURCHASE.REQUEST.APPROVE");
        ...
        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        long requestId = (await new CreatePurchaseRequestHandler(repository, scope.ContextAccessor)
            .HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "Approve audit"))).PurchaseRequestId!.Value;
        await new UpsertPurchaseRequestDetailHandler(repository, scope.ContextAccessor)
            .HandleAsync(new UpsertPurchaseRequestDetailRequest(requestId, null, null, "Line", 1, 100, null));
        await new SubmitPurchaseRequestHandler(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository)
            .HandleAsync(new SubmitPurchaseRequestRequest(requestId));
        ...
        ApprovePurchaseRequestResponse approved = await new ApprovePurchaseRequestHandler(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository)
            .HandleAsync(new ApprovePurchaseRequestRequest(requestId, "approved by second user"));
        ...
        command.CommandText = "SELECT COUNT(1) FROM cumplimiento.auditoria_operacion WHERE tabla='compras.solicitud' AND operacion='APPROVE' AND id_registro=@id;";
    ...
    [Fact]
    public async Task PurchaseRequest_Approve_ShouldPersistSecurityEvent_WhenApproved()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        long approverUserId = await CreateApproverUserAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId);
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.CREATE");
        ...
        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        long requestId = (await new CreatePurchaseRequestHandler(repository, scope.ContextAccessor)
            .HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "Approve event"))).PurchaseRequestId!.Value;
        await new UpsertPurchaseRequestDetailHandler(repository, scope.ContextAccessor)
            .HandleAsync(new UpsertPurchaseRequestDetailRequest(requestId, null, null, "Line", 1, 100, null));
        await new SubmitPurchaseRequestHandler(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository)
            .HandleAsync(new SubmitPurchaseRequestRequest(requestId));
        ...
        ApprovePurchaseRequestResponse approved = await new ApprovePurchaseRequestHandler(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository)
            .HandleAsync(new ApprovePurchaseRequestRequest(requestId, "approved"));
        Assert.True(approved.IsSuccess);
        ...
        await scope.ApplyTenantCompanyContextAsync(connection);
        await using SqlCommand command = connection.CreateCommand();
        command.CommandText = "SELECT COUNT(1) FROM seguridad.security_event_audit WHERE event_type='PURCHASE_REQUEST_APPROVED' AND id_tenant=@t AND id_empresa=@e;";
    ...
    [Fact]
    public async Task PurchaseRequest_DenyByPermission_ShouldBeConsistent()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        (_, ValidateSessionResult session) = await scope.LoginAndSelectCompanyAsync(withMfa: true);
        scope.ContextAccessor.SetCurrent(new RequestContext(session.TenantId, session.CompanyId, session.UserId, session.SessionId, Guid.NewGuid().ToString()));
        ...
        AuthorizationCheckResult decision = await scope.CreateAuthorizationEvaluator().EvaluateAsync(
            new AuthorizationCheckRequest(
                "PURCHASE.REQUEST.APPROVE",
                true,
                "/api/v1/purchase/requests/10/approve",
                "POST",
                "127.0.0.1",
                "SecureERP.Tests",
                Guid.NewGuid().ToString()));
    ...
    [Fact]
    public async Task PurchaseRequest_DenyByMfa_ShouldBeConsistent()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.APPROVE");
        (_, ValidateSessionResult session) = await scope.LoginAndSelectCompanyAsync(withMfa: false);
    ...
    [Fact]
    public async Task PurchaseRequest_DenyBySoD_ShouldBeConsistent()
    {
        await using IamTestScope scope = await IamTestScope.CreateAsync();
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.CREATE");
        await EnsurePermissionAllowAsync(scope.ConnectionString, scope.TenantId, scope.CompanyId, scope.UserId, "PURCHASE.REQUEST.UPDATE");
        ...
        PurchaseRequestRepository repository = new(scope.ConnectionFactory, scope.SessionContextApplier);
        long requestId = (await new CreatePurchaseRequestHandler(repository, scope.ContextAccessor)
            .HandleAsync(new CreatePurchaseRequestRequest(null, DateTime.UtcNow.Date, "SoD consistency"))).PurchaseRequestId!.Value;
        await new UpsertPurchaseRequestDetailHandler(repository, scope.ContextAccessor)
            .HandleAsync(new UpsertPurchaseRequestDetailRequest(requestId, null, null, "Line", 1, 100, null));
        await new SubmitPurchaseRequestHandler(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository)
            .HandleAsync(new SubmitPurchaseRequestRequest(requestId));

        ApprovePurchaseRequestResponse denied = await new ApprovePurchaseRequestHandler(repository, scope.ContextAccessor, scope.CreateOperationalSecurityService(), scope.AuthRepository)
            .HandleAsync(new ApprovePurchaseRequestRequest(requestId, "self approve"));

        Assert.False(denied.IsSuccess);
        Assert.Equal("PURCHASE_REQUEST_SOD_DENY", denied.ErrorCode);
    }
    ...
    private static async Task<long> InsertPurchaseRequestInCompanyAsync(string connectionString, long tenantId, long companyId, long userId)
    {
        await using SqlConnection connection = new(connectionString);
        await connection.OpenAsync();
        await SetSessionContextAsync(connection, "id_tenant", tenantId);
