...
internal sealed class IamTestScope : IAsyncDisposable
{
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        await Exec(connection, "DELETE FROM observabilidad.auditoria_autorizacion WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.security_event_audit WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.contador_rate_limit WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.contador_rate_limit WHERE id_tenant IS NULL AND endpoint IN ('AUTH.LOGIN','AUTH.MFA.CHALLENGE','AUTH.MFA.VERIFY','AUTH.VALIDATE_SESSION','WORKFLOW.APPROVAL_INSTANCE.CREATE','PURCHASE.REQUEST.SUBMIT','PURCHASE.REQUEST.APPROVE','PURCHASE.ORDER.SUBMIT','PURCHASE.ORDER.APPROVE');");
        ...
        await Exec(connection, "UPDATE seguridad.politica_seguridad_operacional SET ventana_segundos=60,max_intentos=12,lockout_minutos=NULL,aplica_lockout=0,activo=1,actualizado_utc=SYSUTCDATETIME() WHERE codigo_accion='PURCHASE.REQUEST.SUBMIT';");
        await Exec(connection, "UPDATE seguridad.politica_seguridad_operacional SET ventana_segundos=60,max_intentos=8,lockout_minutos=NULL,aplica_lockout=0,activo=1,actualizado_utc=SYSUTCDATETIME() WHERE codigo_accion='PURCHASE.REQUEST.APPROVE';");
        await Exec(connection, "UPDATE seguridad.politica_seguridad_operacional SET ventana_segundos=60,max_intentos=12,lockout_minutos=NULL,aplica_lockout=0,activo=1,actualizado_utc=SYSUTCDATETIME() WHERE codigo_accion='PURCHASE.ORDER.SUBMIT';");
        await Exec(connection, "UPDATE seguridad.politica_seguridad_operacional SET ventana_segundos=60,max_intentos=8,lockout_minutos=NULL,aplica_lockout=0,activo=1,actualizado_utc=SYSUTCDATETIME() WHERE codigo_accion='PURCHASE.ORDER.APPROVE';");
        ...
        await Exec(connection, "IF OBJECT_ID(N'compras.solicitud_detalle', N'U') IS NOT NULL AND OBJECT_ID(N'compras.solicitud', N'U') IS NOT NULL DELETE d FROM compras.solicitud_detalle d INNER JOIN compras.solicitud s ON s.id_solicitud=d.id_solicitud WHERE s.id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "IF OBJECT_ID(N'compras.solicitud', N'U') IS NOT NULL DELETE FROM compras.solicitud WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "IF OBJECT_ID(N'compras.orden_compra_detalle', N'U') IS NOT NULL AND OBJECT_ID(N'compras.orden_compra', N'U') IS NOT NULL DELETE d FROM compras.orden_compra_detalle d INNER JOIN compras.orden_compra o ON o.id_orden_compra=d.id_orden_compra WHERE o.id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "IF OBJECT_ID(N'compras.orden_compra', N'U') IS NOT NULL DELETE FROM compras.orden_compra WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.usuario_scope_unidad WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.desafio_mfa WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.sesion_usuario WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.flujo_autenticacion WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.excepcion_permiso_usuario WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        ...
        await Exec(connection, "DELETE FROM seguridad.credencial_local_usuario WHERE id_usuario IN (SELECT id_usuario FROM seguridad.usuario_tenant WHERE id_tenant=@t);", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.usuario_empresa WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
        await Exec(connection, "DELETE FROM seguridad.usuario_tenant WHERE id_tenant=@t;", ("@t", SqlDbType.BigInt, TenantId));
